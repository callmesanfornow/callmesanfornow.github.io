---
permalink: /
title: "Welcome! Everything is fine."
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---
**Research focus:** I build and evaluate methods for detecting abusive and hateful language in low-resource, multilingual and code-switched settings, across speech and text, using pre-trained representations and asking what they actually capture about language, speakers and bias.

Hi! I am a PhD student in Computer Science at the Data Intelligence and Communication Engineering Lab of Télécom SudParis, Institut Polytechnique de Paris, where I am supervised by [Noel Crespi](https://scholar.google.com/citations?user=z_-5wSkAAAAJ) and [Reza Farahbakhsh](https://scholar.google.com/citations?user=HukBnBlWTE0C&). I am a recipient of a fellowship under the Indo-French Center for the Promotion of Advanced Research[CEFIPRA/IFCPAR] for the [ADEPT](https://ifindia.cdn.betanet.in/ifindia/uploads/2024/03/PhD-in-AI-Computer-Science.pdf) project.

My research focuses on the intersection of Speech and Language Technologies and Computational Social Sciences, with an emphasis on hate and offensive content moderation, particularly in low-resource and code-mixed/code-switched languages. I also work in the computational aspects of Digital Humanities, focusing on literary devices, code-switching and its impact on the popularity of new media. 

News
===
**[2026-04-20]** Our paper on region-grounded 3D PET/CT report generation for Vietnamese, with AI4LIFE at Hanoi University of Science and Technology, will appear at ACL 2026. Read it [here](https://arxiv.org/abs/2604.18145). 

**[2026-04-11]** New preprint on few-shot contrastive adaptation (CLAP) for audio abuse detection in low-resource Indic languages. Read it [here](https://arxiv.org/abs/2604.09094). 

**[2025-12-08]** Gave a talk at Total Energies, Paris on K-Pop Code-Switching. Find the slides [here](https://temtsp-my.sharepoint.com/:p:/g/personal/aditya-narayan_sankaran_telecom-sudparis_eu/IQD21RZ3LXIWRbnVpU38At6yAWhlknXhWkk5eMZqQhZeHfw?e=REZ9KK). 

**[2025-12-08]** Headed to Luxembourg for CHR 2025 to present my work on K-Pop Code-Switching. 

**[2025-06-29]** I was part of the Summer Institute in Computational Social Science 2025 at ENSAE, Paris. 

**[2025-02-01]** I officialy start my PhD at Télécom SudParis, Institut Polytechnique de Paris. 

**[2024-11-29]** Our work on Few Shot Audio Abuse Detection was accepted as part of COLING 2025. 

**[2024-05-22]** We presented our work on Gender Stereotypes in Rhymes and Poems at LREC-COLING 2024 

**[2024-05-02]** I recieved a fellowship under the Indo-French Center for the Promotion of Advanced Research[CEFIPRA/IFCPAR] for ADEPT. 